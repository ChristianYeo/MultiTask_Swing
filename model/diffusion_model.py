...
class One_Factor_Hjm_Ln(HJM_LOG_NORMAL):
    ...
    def Simulate_X(self, M):
        ...
        return torch.cumsum(self.diffusion_params['sigma_Z'] * torch.randn((N, M), device = device), 0).unsqueeze(2)
    
    
    def Compute_Spot_Price_From_X(self, X):
        return self.f0 * torch.exp(self.sigma * self.diffusion_params['kappa'] * X.squeeze(2) - 0.5 * self.diffusion_params['lambda'])


# In[ ]:


class Multi_Factor_Hjm_Ln(HJM_LOG_NORMAL):
